# AGENTS.md - AI Agent Development Guide for tap-meteotrentino
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from MeteoTrentino.

### Key Components

1. **Tap Class** (`tap_meteotrentino/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_meteotrentino/client.py`): Handles API communication and authentication
1. **Streams** (`tap_meteotrentino/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_meteotrentino/streams.py`
...
```python
class MyNewStream(MeteoTrentinoStream):
    name = "my_new_stream"
...
1. **SimpleHeaderPaginator**: For APIs using Link headers (RFC 5988)
   ...
   class MyStream(MeteoTrentinoStream):
...
```bash
tap-meteotrentino --config config.json --discover
tap-meteotrentino --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap_meteotrentino/tap.py`
...
```bash
# .env.example
TAP_METEOTRENTINO_API_URL=https://api.example.com
TAP_METEOTRENTINO_API_KEY=your_api_key_here
TAP_METEOTRENTINO_BATCH_SIZE=100  # New setting
```
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-meteotrentino --version`), Python version, and a minimal reproduction case when reporting bugs.
